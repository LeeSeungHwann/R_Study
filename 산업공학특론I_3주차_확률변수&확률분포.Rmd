...
meanvar <- function(dist, n=10000, param){
  ...
  print(paste("simualtion result : ", mean(rand), var(rand)))
  print(paste("산출 공식 : ", m, v))
}
...
dat$Target <- as.factor(dat$Target) #범주형 변경
...
```{r data_group}
dat_split <- split(dat$Tool.wear..min., dat$Failure.Type)
summary(dat_split$`No Failure`)
head(dat_split)
?split
```
